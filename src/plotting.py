...
from bcann_vae import rec_loss_trainable
def plot_vae(vae_model, stretches, stresses, id, input): #
    # Compute reconstructed stress
    output = vae_model.predict(input)
    stress_pred_mean, stress_pred_var = np.split(output, 2, axis=-1)
    # print(stress_pred_mean)
    # print(stress_pred_var)
    # print(stresses)
    # print(stretches)
    terms = False
    plot_dist = False
    blank = False
    stretch_plot = stretches.reshape((3, 100, 2))
    stress_true = stresses.reshape((3, 100, 2))
    stress_pred_mean = stress_pred_mean.reshape((3, 100, 2))
    stress_pred_var = stress_pred_var.reshape((3, 100, 2))
    ...
    axes = flatten([list(x) for x in zip(*axes)])
    direction_strings = ["w", "s"] * 3# + ["x", "y", "x", "y", "x"]
    n_spaces = 55
    titles = ["strip-w", None, "equibiax-ws", None, "strip-s", None]
    inputs = [[stretch_plot[j, :, k] for k in range(2)] for j in range(3)]
    inputs = [[x[i] if np.max(x[i]) > 1.0 else ((x[1 - i] - 1) * 1e-9 + 1) for i in range(2)] for x in inputs]
    inputs = flatten(inputs)  # 100 len each
    outputs = flatten([[stress_true[j, :, k] for k in range(2)] for j in range(3)])
    #
    # if terms:
    #     pred_terms = [stress_pred_terms[i, j, :, k, :] for i in range(2) for j in range(5) for k in range(2)][0:15]
    pred_mean = flatten([[stress_pred_mean[j, :, k] for k in range(2)] for j in range(3)])
    pred_std = flatten([[np.sqrt(stress_pred_var[j, :, k]) for k in range(2)] for j in range(3)])
    cmap = plt.cm.get_cmap('jet_r', 17)  # define the colormap
    cmaplist = [cmap(i) for i in range(cmap.N)]
    ...
    for i in range(len(axes)):
        ...
        # Create plot that fills between the lower and upper bound
        axes[i].fill_between(inputs[i], pred_mean[i] - pred_std[i], pred_mean[i] + pred_std[i], lw=0, zorder=0,
                             color="#384ebc", alpha=0.25,
                             label=i + 1)
        eps = 1e-6
        # Compute negative log likelihood for a normal distribution
        errors = 0.5 * (
                    np.log(2 * np.pi * (pred_std[i] ** 2 + eps)) + (outputs[i][:] - pred_mean[i]) ** 2 / (
                    pred_std[i] ** 2 + eps))  # Result should be 5 x 100
        nll = np.mean(errors)
        print(stresses.shape)
        print(output.shape)
        nll2 = rec_loss_trainable(np.float64(stresses), np.float64(output))
        # if i == i_dev or i == i_test:
        #     print(("Test" if i == i_test else "Dev") + f" Loss: {nll}")
        # else:
        # sf = 0.8
        # nll2 = np.mean(
        #     0.5 * (np.log(2 * np.pi * ((pred_std[i] * sf) ** 2 + eps)) + (outputs[i][:] - pred_mean[i] * sf) ** 2 / (
        #             (pred_std[i] * sf) ** 2 + eps)))  # Result should be 5 x 100
        train_loss += nll
        train_loss2 += nll2
        ...
        if inputs[i].shape[0] > num_points:
            input_old = inputs[i]
            inputs[i] = np.linspace(np.min(input_old), np.max(input_old), num_points)
            outputs[i] = np.interp(inputs[i], input_old, outputs[i])

        if terms or not plot_dist:
            scatterplot = axes[i].scatter(inputs[i], outputs[i][:], s=300, zorder=25, lw=3, facecolors='w',
                                          edgecolors='k',
                                          clip_on=False)
        else:
            data_mean = np.mean(outputs[i], axis=0)
            ...
            errors = 0.5 * (np.log(2 * np.pi * (data_std ** 2 + eps)) + (outputs[i][:, :] - data_mean) ** 2 / (
                    data_std ** 2 + eps))  # Result should be 5 x 100
            nll_min = np.mean(errors)
            axes[i].fill_between(inputs[i], data_mean - data_std_sample, data_mean + data_std_sample, lw=0,
                                 zorder=1,
                                 color="#FF0000", alpha=0.25, label=i + 1)
            axes[i].plot(inputs[i], data_mean, lw=4, zorder=24, color='#FF0000')
    ...
    print(f"Train Loss: {train_loss}, {train_loss2}")
    ...
    if terms:
        ...
        engine.set(rect=(0.005, 0.005, 0.99, 0.99), wspace=0.04)
        for j in range(5):
            rec = plt.Rectangle((0.2 * j - x_offset * (j < 3), rect_height), 0.2 + x_offset * (j == 2),
                                1 - rect_height, fill=False, lw=2)
            rec.set_zorder(1000)
            rec = fig.add_artist(rec)
        for j in range(2):
            rec = plt.Rectangle((0.4 * j - x_offset, 0), 0.4 + x_offset * (j == 1), rect_height, fill=False, lw=2)
    ...
    name = "terms" if terms else ("variance2" if plot_dist else "variance1")
    if blank:
        plt.savefig(f"../Results/vae/raw_data.pdf",
                    transparent=False,
                    facecolor='white')
    else:
        plt.savefig(f"../Results/vae/bcann_{id}.pdf",
                    transparent=False,
                    facecolor='white')
